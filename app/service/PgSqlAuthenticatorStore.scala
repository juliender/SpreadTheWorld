  ...
  def save(authenticator: Authenticator): Either[Error, Unit] = {
    ...
    DB.withConnection { implicit c =>
      ...
      val sqlSelectQuery = SQL(
        ...
        """).on("id" -> authenticator.identityId.userId)

      val authenticators = sqlSelectQuery().map(row =>
        Authenticator(
          row[String]("id"),
          IdentityId(row[String]("userId"), row[String]("provider")),
      ...
      if (foundAuthenticator == None) { // user not exists
        ...
        val sqlQuery = SQL(
          ...
          """).on(
          'id -> authenticator.id,
          'userId -> authenticator.identityId.userId,
          'provider -> authenticator.identityId.providerId,
